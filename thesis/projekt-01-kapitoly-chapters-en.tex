...
\begin{itemize}
    ...
    \item \textbf{PostgreSQL \& Entity Framework Core}: PostgreSQL was chosen over NoSQL alternatives (like MongoDB) or local flat-file databases (like SQLite) for its robust Multi-Version Concurrency Control (MVCC). In this system, the actor network constantly bulk-inserts recovered passwords and telemetry updates. MVCC ensures that these heavy write operations never block the Web UI from reading and displaying real-time data.
    \item \textbf{Blazor Server}: By embedding a Blazor Server application directly alongside the Coordinator's actor system, the architecture eliminates the need for a traditional decoupled REST API and JavaScript frontend. The user interface resides in the exact same memory space as the cluster, querying Grain states locally and pushing updates to the browser instantly via SignalR.
\end{itemize}

\subsection{The Coordinator (Manager) Node}

The Coordinator serves as the cluster leader, state manager, and web server. It hosts the Blazor Server web interface, the central Consul server instance, and connects to the PostgreSQL database. It does not perform cracking tasks itself; instead, it orchestrates the lifecycle of jobs.

\begin{itemize}
    \item \verb|JobManagerGrain|: The root supervisor Grain for all cracking operations. When a new job is submitted via the web UI, it spawns a child coordinator specifically for that task.

    \item \verb|JobCoordinatorGrain| (Virtual Actor / Grain): A unique actor assigned to a specific Job ID. It holds the "Global Cursor" of the keyspace. It is responsible for generating work chunks and tracking their state (Pending, Assigned, or Completed). By implementing this as a Virtual Actor (Grain), we ensure that the job state is always addressable and effectively "single-threaded," naturally preventing race conditions where two workers might attempt to claim the exact same keyspace chunk.
...
\subsection{The Agent (Worker) Node}
...
\begin{enumerate}
    \item \textbf{Job Discovery}: If the Agent has just joined the cluster or if the previous job is finished, it can send a \verb|JobDiscovery| message to the root \verb|JobManagerGrain|, which can respond with a \verb|JobAssignment| message containing a JobID of unfinished pending work, if any is available.
...
\end{verbatim}
    ...
    \item \textbf{Work Request}: The Agent can then request work from the \verb|JobCoordinatorGrain| with the identity matching the assigned JobID via a \verb|WorkRequest| message. This message contains its \verb|AgentID| and the benchmarked hashrate used for adaptive chunk sizing. The hashrate is obtained by running a local Hashcat benchmark for the specific hash type (with the command \verb|hashcat -b -m <mode_id>|). The result of this benchmark is cached locally, ensuring faster availability of the Agent for subsequent tasks of the same type.
...
\end{verbatim}
    ...
    \item \textbf{Assignment}: The \verb|JobCoordinatorGrain| creates a work assignment message based on the type of attack it is orchestrating. To handle assignments effectively for both Brute Force (Mode 3) and Dictionary (Mode 0) attacks, the system employs a polymorphic message design, containing common metadata and additional specialized fields.
...
\end{verbatim}
    ...
    \item \textbf{Completion}: Upon finishing the chunk, the Agent sends a \verb|WorkResult| message back to the \verb|JobCoordinatorGrain|, which safely marks the chunk as \verb|Completed|. If any recovered passwords were found in the chunk, they are sent to the \verb|ResultCollectorGrain|.
...
Unlike Hashcat's local \verb|.restore| file, which is useless if a physical node fails, checkpointing is managed logically by the Coordinator's actor state.

\begin{enumerate}
    \item \textbf{Leasing}: When a chunk is assigned to a worker, it moves from \verb|Pending| to \verb|Assigned| with a timestamp.

    \item \textbf{Timeout Supervision}: The \verb|JobCoordinatorGrain| maintains a timer for every \verb|Assigned| chunk. Because the chunks are dynamically sized to fit an exact time window (e.g., 60 seconds), the Coordinator can easily identify anomalies. If a \verb|WorkResult| is not received within the expected duration (plus a slight buffer margin), the chunk is considered "Lost."
