...
In \textbf{Mask Attacks}, Hashtopolis attempts to mitigate hardware heterogeneity by dynamically sizing blocks of the keyspace based on an agent's benchmarked speed. However, because it relies on a central relational database to calculate, assign, and track these non-overlapping chunks, issuing them to hundreds of polling agents simultaneously often causes severe database lock contention. This architectural bottleneck restricts the platform's ability to scale linearly.
...
\end{verbatim}
    ...
    \begin{enumerate}
        \item \textbf{Handling Mask Attacks (Mode 3)}: This is the "Zero-Latency" path. To safely partition work, the Coordinator evaluates two essential Hashcat metrics: \verb|--keyspace| and \verb|--total-candidates|. The keyspace represents the divisible outer loop of the attack algorithm, defining the discrete units of work. However, because a single keyspace unit might expand into multiple password guesses (e.g. due to internal amplification), the keyspace alone is insufficient for time-based load balancing. By dividing the total candidates by the keyspace, the coordinator derives an \textit{amplification factor}. This allows the system to accurately translate an agent's raw benchmarked hashrate (candidates per second) into an exact number of keyspace units (the algorithmic implementation of this dynamic scaling is further elaborated in Section \ref{sec:dynamic_chunk_sizing}). When a worker requests work, the Coordinator reserves a range starting at its \verb|GlobalCursor|, sized specifically to the agent's hardware capabilities, ensuring the task completes within a strict, predictable time window. Unlike legacy systems like Hashtopolis that rely on database transactions to issue dynamic assignments, the \verb|JobCoordinatorGrain| manages this cursor entirely in-memory. This completely eliminates database lock contention, allowing the orchestrator to scale effortlessly across thousands of agents while ensuring that fast GPUs and slow CPUs complete their proportionally sized tasks simultaneously.
